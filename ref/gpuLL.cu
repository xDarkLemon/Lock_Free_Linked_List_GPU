...
// #define SET_MARKED(p)  ((node *)(((unsigned long long)(p)) | 1))
// #define CLEAR_MARKED(p)   ((node *)(((unsigned long long)(p)) & ~1))
...
// __device__ struct node *createNode() {
...
__global__ void listAdd(int *insertVals) {
	// original, add before head
...
__global__ void listInsert(int *insertVals) {
	// add after head
	unsigned idx = blockIdx.x * blockDim.x + threadIdx.x; 
	...
	if (idx<5)
	{
		...
		do {
			myold = head->next; 
			newnode->next = myold;
			actualold = (struct node *)atomicCAS((unsigned long long *)&head->next, (unsigned long long)myold, (unsigned long long)newnode);  
...
{
	// search the node and return its prev ref
	struct node *cur;
...
__device__ struct node *searchNode(int val, struct node **prev)
{
	// search the node and return its prev ref
	struct node *cur;
	for (struct node *ptr = head; ptr->next; ptr = (struct node *)GET_UNMARKED_REF(ptr->next))
...
// __host__ __device__ struct node *listSearch(int val, struct node **prev)
__device__ struct node *listSearch(int val)
{
	printf("listSearch val: %d\n", val);
	// search the node and return its prev ref in argument
...
__global__ void listPrintHead() {
	struct node *p=head;
	nodePrint(p);
	printf("head ptr: %llu, ",GET_UNMARKED_REF(p));
}
...
__device__ void listPrintRawDev() {
	printf("listPrintRawDev\n");
	int nnodes = 0;
	for (struct node *ptr = head; ptr; ptr = (struct node *)GET_UNMARKED_REF(ptr->next))
	{	
		// printf("ptr: %llu, ",GET_UNMARKED_REF(ptr));
...
__global__ void findNode(int val)
{
	printf("findNode\n");
	struct node* p;
	p = listSearch(val);
	printf("\nFind node %d\nunmarked addr: %llu, marked addr: %llu, data: %d\n", val, GET_UNMARKED_REF(p), GET_MARKED_REF(p), ((struct node *)GET_UNMARKED_REF(p))->data);
	// struct node* ptr = (struct node *)GET_UNMARKED_REF(NULL);
	// if(!ptr)
	// 	printf("ptr is null\n");
	// else
	// 	printf("ptr ref: %llu\n", GET_UNMARKED_REF(ptr));
}

void Demo() {
	// cudaMalloc((void **)&head, sizeof(struct node *));  // work
	// cudaMemset(&head, 0, sizeof(struct node *));  // original
	listInit<<<1,1>>>();
	addFront<<<1,1>>>(3);
	addFront<<<1,1>>>(2);
	addFront<<<1,1>>>(1);
